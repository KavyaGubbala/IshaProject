...
public class P2_3VerifyFullRegistratnOverseas extends BaseClass {
	...
	public void testPriority1() throws IOException, InterruptedException {
		...
		for (int i = 0; i < formData.length; i++) {
			...
			Thread.sleep(3000);
			if (!driver.findElements(By.xpath("popup_xpath")).isEmpty()) {
			    // If popup is present, close it
				System.out.println("Nurture popup appeared");
			    WebElement popup = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='Sadhguru-IEO-US-Nurture-UAT-c-canvas Campaign__canvas']")));
			    driver.switchTo().activeElement();
			    WebElement click_crossButton= driver.findElement(By.xpath("//button[@title='Close']"));
			    click_crossButton.click();
			    System.out.println("Popup closed.");
			} else {
			    // If popup is not present, continue execution
			   // System.out.println("No popup found, continuing execution...");
